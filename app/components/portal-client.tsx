...
export function PortalClient({ slug, name, description, faqs }: Props) {
  ...
  return <main className="evac-theme">
    <header className="site-header"><Link className="brand" href={`/${slug}`}><span className="brand-mark" aria-hidden="true">相</span>相談窓口</Link><nav><a href="#faq">FAQを探す</a><a href="#ask">質問する</a><Link href={`/${slug}/admin`}>管理画面</Link></nav></header>
    {/* 出自表示: 窓口の作成には管理者パスワード認証が必須になったため、
        「利用者が自分で勝手に開設した非公式な窓口」という前提はもう成り立たない。
        ただし、避難者が「今どの窓口に相談しているか」を見失わないよう、
        折りたたまず・質問フォームより前の常時見える位置に窓口名を明示する。
    ...
    <section className="portal-hero"><p className="eyebrow">避難所の相談窓口</p><h1>{name}の相談窓口</h1><p>{description || '気になることは、思ったままの言葉で聞けます。'}</p><div className="hero-actions"><a className="button primary" href="#faq">FAQを探す</a><a className="button accent" href="#ask">質問する</a></div></section>
