...
export function PortalClient({ slug, name, description, faqs }: Props) {
  ...
  async function submitQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setQuestion(''); setPreview(null); setStatus('質問を受け付けました。回答者が確認し、FAQ改善のため継続保存されます。');
    } catch { setStatus('通信に失敗しました。時間をおいて再度お試しください。'); }
    finally { setBusy(false); }
  }
  return <main>
    ...
    <section className="section" id="faq"><div className="section-heading"><div><p className="eyebrow">公開FAQ</p><h2>まず、聞く前に探せます</h2></div><label className="search-label"><span className="sr-only">FAQを検索</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例：急に見学に行っても大丈夫？" maxLength={100} type="search" /></label></div>
