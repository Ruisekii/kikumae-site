...
export function OwnerDashboard({ signOutHref }: { signOutHref?: string }) {
  ...
  async function deleteSelected() {
    ...
    try {
      ...
      setDeleteTarget(null);
      setConfirmName('');
      setMessage('窓口を削除しました。関連データも削除済みです。');
    } catch {
      setMessage('窓口の削除に失敗しました。');
    } finally {
      setBusy(false);
    }
  }
  ...
  return <main className="owner-page"><header className="site-header"><Link className="brand" href="/">🐣 きくまえ</Link><div className="header-actions"><Link href="/">公開ページへ</Link><Link href="/admin">管理画面</Link>{signOutHref && <Link href={signOutHref}>ログアウト</Link>}</div></header><section className="owner-content"><p className="eyebrow">きくまえ運営管理</p><h1>作成済みの窓口</h1><p className="owner-note">運営者だけが確認できる一覧です。一般ユーザーや各窓口の管理者には公開されません。</p><form className="owner-toolbar" onSubmit={submitSearch}><label htmlFor="owner-search">窓口名を検索</label><div className="owner-search-row"><input id="owner-search" value={query} onChange={(event) => setQuery(event.target.value)} maxLength={80} placeholder="団体名・窓口名" /><button className="button secondary" type="submit" disabled={busy}>検索</button>{activeQuery && <button className="text-button" type="button" onClick={() => { setQuery(''); setActiveQuery(''); void loadPortals(''); }} disabled={busy}>すべて表示</button>}</div></form><div className="owner-summary"><strong>表示中の窓口 {portals.length}件</strong><span>{resultLabel}</span></div>{message && <p className="form-status" role="status">{message}</p>}{loaded && <div className="owner-list">{portals.map((portal) => <article className="owner-row" key={portal.id}><div><h2>{portal.name}</h2><p className="owner-created">作成：{dateFormatter.format(new Date(portal.createdAt))}</p><code>/{portal.slug}</code></div><div className="owner-actions"><Link className="button secondary" href={`/${portal.slug}`} target="_blank" rel="noreferrer">公開ページを見る</Link><button className="button danger" type="button" onClick={() => openDelete(portal)}>削除</button></div></article>)}{!portals.length && <p className="owner-empty">該当する窓口はありません。</p>}</div>}{deleteTarget && <div className="delete-confirm owner-delete-confirm" role="dialog" aria-modal="true" aria-labelledby="owner-delete-title"><h2 id="owner-delete-title">「{deleteTarget.name}」を削除しますか？</h2><p>この操作は取り消せません。公開FAQ・質問・回答・FAQ候補・確認用データなど、この窓口に紐づくデータも削除されます。</p><label htmlFor="owner-confirm-name">確認のため窓口名を入力してください<input id="owner-confirm-name" value={confirmName} onChange={(event) => setConfirmName(event.target.value)} autoComplete="off" placeholder={deleteTarget.name} /></label><div className="form-actions"><button className="button danger" type="button" disabled={busy || confirmName !== deleteTarget.name} onClick={() => void deleteSelected()}>完全に削除する</button><button className="button secondary" type="button" disabled={busy} onClick={closeDelete}>キャンセル</button></div></div>}</section></main>;
}
