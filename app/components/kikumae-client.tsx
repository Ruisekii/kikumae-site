...
export function KikumaeClient({ faqs }: Props) {
  ...
  return <main className="evac-theme">
    <header className="site-header"><a className="brand" href="#home" aria-label="ホームへ戻る"><span className="brand-mark" aria-hidden="true">相</span>相談窓口</a><nav aria-label="主要メニュー"><a href="#faq">よくある質問</a><Link href="/questions">相談状況</Link><Link href="/admin">職員用</Link></nav></header>
    <section className="shelter-hero" id="home"><div className="shelter-hero-copy"><p className="eyebrow">避難所の相談窓口</p><h1>困っていることを、<br /><span>教えてください。</span></h1><p>言葉がまとまっていなくても大丈夫です。AIがいったん受け止め、必要なことを職員へ分かりやすく届けます。</p><div className="shelter-actions"><button className="button accent large" type="button" onClick={() => beginChat()}><span aria-hidden="true">▱</span> 文字で相談する</button><button className="button secondary large" type="button" onClick={() => beginChat(true)}><span aria-hidden="true">◉</span> 音声で話す</button></div><p className="shelter-reassure">相談は匿名です。原文もそのまま保存され、職員が確認します。</p></div><div className="shelter-hero-note"><span className="note-icon" aria-hidden="true">＋</span><div><strong>あなたの声を、<br />職員に届けます</strong><p>受付後は、対応状況をいつでも確認できます。</p></div></div></section>
    ...
    <nav className="mobile-bottom-nav" aria-label="モバイルメニュー"><a href="#home"><span aria-hidden="true">⌂</span><span>ホーム</span></a><a href="#faq"><span aria-hidden="true">?</span><span>お知らせ</span></a><Link href="/questions"><span aria-hidden="true">◷</span><span>マイページ</span></Link></nav>
  </main>;
}
