...
// 相談本文は保存せず、ID・確認URL・タイトル・保存日時のみを保存する。
...
export function KikumaeClient({ faqs }: Props) {
  ...
  async function submitQuestion() {
    ...
    try {
      ...
      if (id != null && url) appendConsultHistory({ id, checkUrl: url, title: preview.analysis.title, savedAt: Date.now() });
  ...
  return <main className="evac-theme">
    ...
    <section className="shelter-hero" id="home"><div className="shelter-hero-copy"><p className="eyebrow">避難所の相談窓口</p><h1>困っていることを、<br /><span>教えてください。</span></h1><p>言葉がまとまっていなくても大丈夫です。AIがいったん受け止め、必要なことを職員へ分かりやすく届けます。</p><div className="shelter-actions"><button className="button accent large" type="button" onClick={() => beginChat()}><span aria-hidden="true">▱</span> 文字で相談する</button>{voiceSupported && <button className="button secondary large" type="button" onClick={() => beginChat(true)}><span aria-hidden="true">◉</span> 音声で話す</button>}</div><p className="shelter-reassure">相談は匿名です。原文もそのまま保存され、職員が確認します。</p></div><div className="shelter-hero-note"><span className="note-icon" aria-hidden="true">＋</span><div><strong>あなたの声を、<br />職員に届けます</strong><p>受付後は、対応状況をいつでも確認できます。</p></div></div></section>
