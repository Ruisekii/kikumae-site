...
// admin・api・open・owner等はルーティング衝突を防ぐための予約語。窓口（避難所）の
// slugとして使わせない。
...
export async function POST(request: Request): Promise<Response> {
  ...
  // 避難所の登録は無認証の自由開設をやめ、管理者ログイン済みの職員だけに許可する。
  // なりすまし対策の語のブロックリストは、この認証ゲートに置き換えて廃止した。
  const user = await getAdminUser();
  if (!user) return Response.json({ message: '避難所の登録には管理者としてのログインが必要です。/admin から管理者用パスワードでログインしてから、もう一度お試しください。' }, { status: 401, headers: { 'Cache-Control': 'no-store' } });
  ...
  try {
    ...
    if (name.length < 2 || name.length > 80) return Response.json({ message: '避難所の名称は2〜80文字で入力してください。' }, { status: 400 });
